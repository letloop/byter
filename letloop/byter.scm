(library (letloop byter)
  ...
  (define integer->bytevector
    (lambda (integer)
      (assert (and (exact integer) (< 0 integer)))
  ...
  (define byter-negative-integer-pack
    (lambda (accumulator integer)
      ...
      (let loop ((index zero-count-from-the-left))
        (unless (fx=? index 8)
          (accumulator (bitwise-xor (bytevector-u8-ref bytevector index)
                                    #xFF))
          (loop (fx+ index 1))))))
  ...
  (define byter-negative-integer-unpack
    (lambda (bytevector index)
      ...
      (let loop ((index (fx+ index 1))
                 (other start))
        (unless (fx=? index end)
          (bytevector-u8-set! out other (bitwise-xor (bytevector-u8-ref bytevector index)
                                                     #xFF))
          (loop (fx+ index 1) (fx+ other 1))))
      (values (- (bytevector->integer out)) (fx+ index length 1))))
  ...
  (define byter-pack
    (case-lambda
      ((object)
       (byter-pack object (bytevector-accumulator)))
      ((object accumulator)
       (cond
        ((pair? object)
         (accumulator byter-pair)
         (byter-pack (car object) accumulator)
         (byter-pack (cdr object) accumulator))
        ...
        ((vector? object)
         ...
         (vector-for-each
          (lambda (object) (byter-pack object accumulator))
          object)
         (accumulator byter-vector-end))
        (else (error 'byter-pack "Unsupported type" object)))
    ...
    (lambda (bytevector index)
      (let ((tag (bytevector-u8-ref bytevector index)))
        (case tag
          ...
          (else (error 'byter-unpack "Unsupported type with tag" (number->string tag 16)))))))
